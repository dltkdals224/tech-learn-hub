...
매개변수로 몇개의 인수가 전달될 지 모르는 경우, \*args를 사용할 수 있다. <br/>
...
파이썬에서 함수의 리턴 값은 반드시 하나.  
위의 경우처럼 2개 이상의 값을 리턴하려고 하는 경우, 튜플로 반환한다.
...
다만, 매개변수로 (name, age, man = True)는 되지만 (name, man = True, age)는 불가능하다.  
초기화시키고 싶은 매개변수는 항상 뒤쪽에 놓아야 함.

그렇지 않을 경우, SyntaxError가 발생.
