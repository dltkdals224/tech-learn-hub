...
8진수(octal)를 만들기 위해서는 숫자가 0o 또는 0O으로 시작. <br/>
16진수(hexadecimal)를 만들기 위해서는 0x로 시작. <br/>
하지만 python에서 8진수와 16진수는 잘 사용되지 않는다.
...
print(target) # "hi" <- 변수의 값 자체를 변경하는 것이 아님
# upper뿐만 아니라 lower, join, lstrip, rstrip, strip, replace, split 등의 함수는 원본을 변경하지 않는다.
...
del 함수는 파이썬이 자체적으로 가지고 있는 **삭제 함수**이며 모든 **객체**에 대해 사용할 수 있다.<br/>
...
target.sort(key=len) # sort() 함수는 원본 리스트를 변경함.
...
target_list.sort(key=lambda p: p[1]) # 나이를 기준으로 기본(오름차순) 정렬
...
sorted_list = sorted(target_list, key=lambda x: (len(x), x)) # 길이 -> 알파벳 우선순위로 정렬
print(sorted_list) # ['fig', 'kiwi', 'apple', 'banana', 'orange']
```
...
튜플은 요소의 값을 변화시킬 수 없다는 점만 제외하면 리스트와 완전히 동일하다.
...
다른 대부분의 언어도 대응 관계를 나타내는 자료형을 갖고 있는데,  
이를 연관 배열(Associative array) 또는 해시(Hash)라고 하며, 파이썬에서는 딕셔너리(Dictionary)라고 한다.

딕셔너리는 Key와 Value를 한 쌍으로 갖는 자료형.

딕셔너리는 리스트나 튜플처럼 순차를 근간으로 값을 구하지 않고 Key를 통해 Value를 얻는다. <br/>
...
리스트나 튜플, 문자열은 요솟값을 얻고자 할 때 인덱싱이나 슬라이싱을 사용. <br/>
...
또 한 가지 주의해야 할 사항은 Key에 리스트는 사용할 수 없다.  
하지만 튜플은 Key로 사용할 수 있다.  
딕셔너리의 Key로 사용가능 여부는 Key가 변하는(mutable) 값인지 변하지 않는(immutable) 값인지에 달려 있다.
...
print(list(dic.values()))   # ['sangmin', '010-0000-9866', '0224']
```
...
print(list(dic.items()))   # [('name', 'sangmin'), ('phone', '010-0000-9866'), ('birth', '0224')]
```
...
(여기에서 None은 "거짓"이라는 뜻이라고 알아두자.)

<br/>

> 임의 Key가 딕셔너리에 존재하는지 조사하기(in)

```python
dic = {'name': 'sangmin', 'phone': '010-0000-9866', 'birth': '0224'}

print('name' in dic)  # True → O(1)
print('phone' in dic)  # True → O(1)
```

<br/>

> 임의 Value가 딕셔너리에 존재하는지 조사하기(in)

```python
dic = {'name': 'sangmin', 'phone': '010-0000-9866', 'birth': '0224'}

print('sangmin' in dic.values())  # True → O(n)
print('0101' in dic.values())  # False → O(n)

# value도 O(1)로 탐색하고 싶다면, 역방향 dict를 생성 후 탐색.
reverse_dic = {v: k for k, v in dic.items()}  # 이 과정 자체가 O(n)

print('sangmin' in reverse_dic)  # True → O(1)
```
...
만약 set 자료형에 저장된 값을 인덱싱으로 접근하려면, 다음과 같이 리스트나 튜플로 변환 후 접근해야 한다.
...
표에 None이 있는데, 그저 None은 거짓을 뜻한다는 것만 알아두자.
...
print(id(a)) # 3235344526784
print(id(a)) # 3235348214080
