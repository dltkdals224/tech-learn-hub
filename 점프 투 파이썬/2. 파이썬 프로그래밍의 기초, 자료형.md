...
파이썬에서는 이러한 자료형을 딕셔너리(Dictionary)라고 한다. 단어 그대로 해석하면 사전.
...
baseball이라는 단어의 뜻을 찾기 위해 사전의 내용을 순차적으로 모두 검색하는 것이 아니라 <br/>
...
딕셔너리에서 중복되는 Key 값을 설정해 놓으면 하나를 제외한 나머지 것들이 모두 무시된다는 점을 주의.
