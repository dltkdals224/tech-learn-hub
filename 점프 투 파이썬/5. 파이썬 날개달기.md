...
반복 가능한(iterable) 데이터를 입력으로 받아 하나라도 참이 있으면 True를 리턴하고, 모두 거짓일 때 False를 리턴한다.
...
그리고 반복 가능한 데이터(iterable)의 요소 순서대로 함수를 호출했을 때 반환 값이 참인 것만 걸러내 리턴한다.
...
int(x)는 문자열 형태의 숫자나 소수점이 있는 숫자를 정수로 리턴하는 함수이다. 만약 정수가 입력되면 그대로 리턴한다. <br/>
...
min(iterable) 함수는 인수로 반복 가능한 데이터를 입력받아 그 최솟값을 리턴하는 함수이다.
...
ord(c)는 문자의 유니코드 숫자 값을 리턴하는 함수이다. (chr(i)와 반대)
...
round(number[,ndigits]) 함수는 숫자를 입력받아 반올림해 리턴하는 함수이다. <br/>
...
리스트 자료형의 sort 함수와 다른 형태의 사용법. <br/>
리스트 자료형의 sort 함수는 리스트 객체 그 자체를 정렬할 뿐, 결과를 리턴하지는 않는다.
...
- random.random(): 다음은 0.0에서 1.0 사이의 실수 중에서 난수 값을 리턴.
- random.randint(): 주어진 범위의 정수 사이에서 난수 값을 리턴.
