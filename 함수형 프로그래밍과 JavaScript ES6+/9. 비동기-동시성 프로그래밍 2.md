...
// 다음의 코드들은 모두 즉시 동작함.
C.map((target) => delay1000ms(target * target), [1, 2, 3, 4]).then(console.log); // [ 1, 4, 9, 16 ]
C.filter((target) => delay1000ms(target % 2), [1, 2, 3, 4]).then(console.log); // [ 1, 3 ]
```
