...
더이상 재귀호출을 하지 않아도 계산값을 변환할 수 있는 상황을 말한다. <br/>
...
O(n) n에 비례한 호출: f(n) = f(n-1) + n

O(2^n) 2^n에 비례한 호출: f(n) = f(n-1) + f(n-3)
...
모든 노드의 차수가 n개 이하인 트리를 n진 트리라고 함. (보통 이진 트리(Binary Tree)의 사용이 많음)
